...
def browse_folders(input_folder_path: StringVar):
...
def get_args_gui() -> dict:
    ...
    # input_folder_path
    lbl = Label(root, text="Insert path to the video folder:")
    lbl.grid(column=0, row=0)
    input_folder_path = StringVar(root, value='Select folder')
    Entry(root, width=30, textvariable=input_folder_path).grid(column=1, row=0)

    button_explore = Button(root, text="Browse", command=browse_folders(input_folder_path))
    button_explore.grid(column=2, row=0)

    lbl = Label(root, text="Keep original files?")
    lbl.grid(column=0, row=1)
    keep_original_int = IntVar(root, value=0)
    Radiobutton(root, text="Yes", value=1, variable=keep_original_int).grid(column=1, row=1)
    Radiobutton(root, text="No", value=0, variable=keep_original_int).grid(column=2, row=1)
